...
__device__ inline float hitgrid(float3* p0, float3* v, float* rv, short id[4]) {
    ...
    htime[1] = fabs((id[1] + (v->y > 0.f) - p0->y) * rv[1]);
    htime[2] = fabs((id[2] + (v->z > 0.f) - p0->z) * rv[2]);
        ...
        if (!ispencil) {
            switch (gcfg->srctype) {
                     ...
                     // @param[in,out] rv: the reciprocal direction vector of the photon (rv[i]=1/v[i])
                {
                    ...
                    *((float4*)v)=float4(
                             -sin_ang * length,
                             sinf(illumination_angle) * length,
                             -cos_ang * length,
                             v->nscat
                        );
				...
				case (MCX_SRC_DIFFUSE): { // diffuse beam for being able to compute sphere corrections for the DIS setup
				
                    float ang, stheta, ctheta, sphi, cphi, r;
					
					// translate the photon launch vector uniformly on disk.
                    ang = TWO_PI * rand_uniform01(t);
                    sincosf(ang, &sphi, &cphi);
					r = sqrtf(rand_uniform01(t) * (gcfg->srcparam1.x * gcfg->srcparam1.x));					
					*((float4*)p)=float4(
									p->x += r * cphi,
									p->y += r * sphi,
									p->z,
					...
					// polar angle is defined as 2 * sin(theta) * cos(theta)
                    ang = acosf(1.0f-2.0f*rand_uniform01(t))/2.0f;
                    sincosf(ang, &stheta, &ctheta);
					
					//azimuthal angle is defined uniform
                    ang = TWO_PI * rand_uniform01(t); 
                    sincosf(ang, &sphi, &cphi);
					
					// rotate the photon launch vector.
                    rotatevector(v, stheta, ctheta, sphi, cphi);
        ...
        if (isreflect) {
            ...
            if (issvmc && hitintf) {
                ...
                if (((isreflect && (isdet & 0xF) == 0) || (isdet & 0x1)) && ((isdet & 0xF) == bcMirror || n1 != ((gcfg->mediaformat < 100) ? (prop.n) : (gproperty[(mediaid > 0 && gcfg->mediaformat >= 100) ? 1 : mediaid].w)))) {
                    ...
                    tmp0 = n1 * n1;
                    tmp1 = prop.n * prop.n;
                    cphi = fabs( (flipdir[3] == 0) ? v.x : (flipdir[3] == 1 ? v.y : v.z)); // cos(si)
