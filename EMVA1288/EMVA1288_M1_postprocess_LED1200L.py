# EMVA1288_M1_postprocess_LED1200L is to process measurements of EMVA1288 (1) sensitivity, (2) temporal noise, (3) linearity
# at 1200 nm, with LED1200L illumination spectrum
...
if __name__ == '__main__':
    ...
    LED1200L_wave = np.loadtxt(PATH + "/Light Sources/LED1200L.txt", usecols=0)
    LED1200L_counts = np.loadtxt(PATH + "/Light Sources/LED1200L.txt", usecols=1)

    """Import EMVA M1 Measurement Variables"""
    meas_var = np.load(f"{PATH}/M1Data/measurement.npz")
    E = 2.06  # Todo: a different reading from power meter when LED1200L was loaded
    A = meas_var['A']
    ...
    plt.plot(t_exp_dist, mup_dist, label="Original E=1.9")
    plt.plot(t_exp_dist, mup_dist_1, label="Original E=2.06")
    ...
    """20230702: Update Linear Regression to Estimate Uncertainty in R"""
    x = mup_dist
    y = muy - muy_dark
    # fit: 0-70% saturation range
    lower_limit_idx = 0
    upper_limit_idx = np.argmax(y > 0.7 * y[-1])
    x = x[lower_limit_idx:upper_limit_idx]
    y = y[lower_limit_idx:upper_limit_idx]
    res = stats.linregress(x, y)
    ts = tinv(0.05, len(x) - 2)
    print(f"R slope (95%): {res.slope:.6f} +/- {ts * res.stderr:.6f}")

    """Calculate K and Plot Photon Transfer"""
    K = EMVA1288_M1_utils.calc_K(muy, muy_dark, sigmay2, sigmay_dark2)
    """20230702: Update Linear Regression to Estimate Uncertainty in K"""
    x = muy - muy_dark
    ...
    res = stats.linregress(x, y)
    ts = tinv(0.05, len(x) - 2)
    print(f"K slope (95%): {res.slope:.6f} +/- {ts * res.stderr:.6f}")
